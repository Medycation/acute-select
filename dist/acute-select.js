...
acFocus.$inject = ['$timeout', '$parse', '$log', 'safeApply'];
function acFocus($timeout, $parse, $log, safeApply) {
    var directive = {
        restrict: 'A',
...
acOnScroll.$inject = ['$log'];
function acOnScroll($log) {
    var directive = {
        restrict: 'A',
...
function acSelect(acuteSelectService) {
  ...
  var directive = {
    ...
    //                          CONTROLLER
    // **************************************************************
    controller: acSelectController
  };
  return directive;
}

acSelectController.$inject =
  ['$scope', '$element', '$window', '$rootScope', '$timeout', '$filter', '$log', 'navKey', 'safeApply', 'acuteSelectService'];
function acSelectController($scope, $element, $window, $rootScope, $timeout, $filter, $log, navKey, safeApply, acuteSelectService) {
        ...
        if (item) {
          ...
          if (!$scope.searchText) {
            ...
            if (dataItem === selectedDataItem ||
              (key && selectedDataItem && dataItem[key] === selectedDataItem[key])) {
  ...
  $scope.getItemClass = function (item) {
    if ($scope.selectedItem && item.value === $scope.selectedItem.value) {
...
angular.module("acute.select").run(['$templateCache', function(a) { a.put('/acute.select/template.html', '<div class="ac-select-wrapper" ng-class="{\'ac-select-wrapper-focused\': popupVisible}" ng-keydown="keyHandler($event)" tabindex="999" ac-focus="wrapperFocus" ng-focus="comboFocus = true">\n' +
 ...
 '            <ul>\n' +
 '                <li id="{{item.id}}" ng-repeat="item in items | filter: search | orderBy:\'text\'" ng-class="getItemClass(item)" ng-click="itemClick(item)" ng-style="{ height: settings.itemHeight + \'px\', \'line-height\': settings.itemHeight + \'px\' }">\n' +
 '                    {{item.text}}\n' +
 '                </li>\n' +
 '            </ul>\n' +
